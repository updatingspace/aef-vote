...
- `backend` — собирается из `backend/Dockerfile`, запускает миграции и `runserver 0.0.0.0:8000`. Код монтируется, секрет читается из `/run/secrets/django_secret_key`, база — PostgreSQL (или SQLite, если `POSTGRES_HOST` пуст). Build-аргумент `INSTALL_DEV` ставит dev-зависимости для локальной разработки.
...
- После старта фронт доступен по `https://aef-vote.updspace.com`, а API работает под `https://aef-vote.updspace.com/api/`, потому что в `stack.yml` заданы маршруты через Traefik и backend доступен только по префиксу `/api`.
